<template>
  <div class="fixed top-0 bg-[#00000046] left-0 w-full h-full z-10 flex items-center justify-center">
    <div class="absolute top-0 left-0 w-full h-full" @click="show_deleteDriver(false)"/>
    <div class="relative bg-white text-center rounded-md p-6 flex flex-col gap-4 w-[350px]">
      <div>
        ...
        <p class="text-sm text-[#6B7280]">
          Are you sure you want delete driver?
        </p>
      </div>
      <div class="flex gap-3">
        <Button white full @click="show_deleteDriver(false)">
          No
        </Button>
        <Button :disabled="disabledDelete" full @click="deleteDriver">
          Yes
        </Button>
      </div>
    </div>
  ...
  methods: {
    ...
    async deleteDriver() {
      if(!this.disabledDelete) {
        ...
        try {
          this.loading_alert()
          await this.delete_driver(this.activeDriver.id)
          this.success_alert('The driver has been successfully deleted')
        } catch(err) {
          const errors = []
