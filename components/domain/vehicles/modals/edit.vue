<template>
  <div class="fixed top-0 bg-[#00000046] left-0 w-full h-full z-10 flex items-center justify-center">
    <div class="absolute top-0 left-0 w-full h-full" @click="show_editVehicle(false)"/>
    <div class="relative overflow-y-auto bg-white max-h-[90%] h-[90%] w-[90%] md:w-[960px] rounded-md flex flex-col justify-between">
      <div class="px-[52px] pt-[52px] pb-7 h-full">
        ...
        <div v-else>
          <Tabs class="my-6" :list="listTabs" :active="activeTab"
          @click="clickItem"/>
      ...
      <div class="bg-[#B8D5FE] flex gap-4 justify-end py-3 px-6 sticky bottom-0 left-0">
        <Button white medium @click="show_editVehicle(false)">
          Cancel
        </Button>
...
export default {
  ...
  methods: {
      ...
      if(this.activeTab == 1) {
        ...
        if(!this.disabledStep) {
          ...
          try {
            ...
            this.success_alert('The vehicle has been successfully edit')
            this.activeTab = 2
          } catch(err) {
            const errors = []
    ...
    async getVehicle() {
      await this.get_vehicle(this.activeVehicle.id)
