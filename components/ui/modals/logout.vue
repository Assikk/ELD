<template>
  <div class="fixed top-0 bg-[#00000046] left-0 w-full h-full z-10 flex items-center justify-center">
    <div class="bg-white text-center rounded-md p-6 flex flex-col gap-4 w-[350px]">
      <div>
      ...
      <div class="flex gap-3">
        <Button white full @click="show_logout(false)">
          No
        </Button>
        <Button full @click="logout" :disabled="disabledLogout">
          Yes
        </Button>
      </div>
    </div>
    ...
    async logout() {
      if(!this.disabledLogout) {
        ...
        try {
          ...
          this.success_alert('The exit was completed successfully')
